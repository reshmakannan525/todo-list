...
import { BsFillCalendarWeekFill } from 'react-icons/bs'; import {AiFillDelete, AiFillEdit} from 'react-icons/ai'; import {RiSave3Line} from 'react-icons/ri';
import { useState } from "react";
import toast, { Toaster } from 'react-hot-toast';
const TasksList = ({data}) => {
const [jsonData, setData] = useGlobalState('data'); const [editingId, setEditingId] = useState(null); const [editing, setEditing] = useState(false);
const [newName, setNewName] = useState(''); const [newContent, setNewContent] = useState('');
...
{(!editing || task.id !== editingId ) && <h2 id={"name-" +
task.id}>{ newName === "" ? task.name : (editing && task.id === editingId ?
newName : task.name)
}</h2>}
...
{setEditingId(task.id);setEditing(!editing);	setNewName(task.name); setNewContent(task.body)}}><AiFillEdit></AiFillEdit> Edit</button>}

{editing && task.id === editingId && <button onClick={() =>
{ if(isEmptyOrSpaces(newContent) ||isEmptyOrSpaces(newName)){ toast.error('Fill the blank fields');
}else{
editTaskContent(task.id, newContent, newName); setEditingId(null);
setEditing(!editing); setData(getTasks());
}
